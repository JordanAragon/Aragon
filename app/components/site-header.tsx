...
export default function SiteHeader() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      { rootMargin: '-24% 0px -60% 0px', threshold: [0.05, 0.18, 0.4, 0.65] },
    );
  ...
  return (
    <>
      <header className="site-header-v2 v8-header" data-scrolled={active !== 'inicio' || undefined} data-ready={entryStarted || undefined}>
        <a href="#inicio" className="brand-v2" onClick={() => setMenuOpen(false)} aria-label="Aragon, inicio" tabIndex={entryStarted ? 0 : -1}>
          ...
          <span><strong>ARAGON</strong><small>SOFTWARE &amp; TECHNOLOGY STUDIO</small></span>
        </a>
      ...
      <div className={'mobile-nav-v2-shell ' + (menuOpen ? 'is-open' : '')} data-open={menuOpen || undefined}>
        <nav id="mobile-nav-v2" ref={menuRef} className="mobile-nav-v2 v8-mobile-nav" aria-label="Menú móvil" aria-hidden={!menuOpen}>
          <div className="mobile-nav-v2-head"><span>ARAGON / INDEX</span><span>{active.toUpperCase()}</span></div>
          {navItems.map(([id, label], index) => (
            <a key={id} href={'#' + id} onClick={() => setMenuOpen(false)} tabIndex={menuOpen && entryStarted ? 0 : -1}>
              <span>{label}</span><b>0{index + 1}</b>
            </a>
          ))}
          <a href="#lab" onClick={() => setMenuOpen(false)} tabIndex={menuOpen && entryStarted ? 0 : -1}><span>Lab</span><b>08</b></a>
